...
def test_default_get_default_returns_array_when_not_expanded(default_row, param_dataset):
...
def test_default_set_value_uniform_no_fixed(default_row, param_dataset, working_param_dataset):
...
def test_default_set_value_with_fixed_indices(default_row, param_dataset, working_param_dataset):
...
def test_default_set_value_sets_single_slot(default_param, working_param_dataset, param_dataset):
...
def test_default_set_value_with_active_index(default_row, param_dataset, working_param_dataset):
    ...
    assert working_param_dataset["fates_leaf_slatop"].values[1] == pytest.approx(0.099)  # written
    assert working_param_dataset["fates_leaf_slatop"].values[2] == pytest.approx(
        0.030
    )  # unchanged
...
def test_default_set_value_write_at_index_rejects_array_value(
    ...
    with pytest.raises(ValueError, match="scalar"):
        default_param.set_value(working_param_dataset, param_dataset, np.array([0.01, 0.02]))
...
def test_default_set_value_array_value(default_param, working_param_dataset, param_dataset):
    ...
    np.testing.assert_array_equal(working_param_dataset["fates_leaf_slatop"].values, value)
...
def test_default_set_value_fixed_indices_held_at_default(
    ...
    default_param.set_value(working_param_dataset, param_dataset, 0.025, {"fates_pft": [1]})
...
def test_default_set_value_scalar_param(scalar_param, working_param_dataset, param_dataset):
    ...
    assert float(working_param_dataset["fates_canopy_closure_thresh"].values) == pytest.approx(0.8)


def test_default_set_value_ds_not_mutated(default_param, working_param_dataset, param_dataset):
    """DefaultValue.set_value doesn't mutate the default dataset"""
    original = param_dataset["fates_leaf_slatop"].values.copy()
    default_param.set_value(working_param_dataset, param_dataset, 0.025, {"fates_pft": [0]})
...
def test_default_set_value_array_passthrough(default_row, param_dataset, working_param_dataset):
    """DefaultParameter.set_value writes can pass an array"""
    param = Parameter.from_row(default_row, param_dataset)
    working_param_dataset["fates_leaf_slatop"].values = np.array([1.0, 2.0, 3.0])
    param.set_value(working_param_dataset, param_dataset, value=np.array([4.0, 5.0, 6.0]))
    np.testing.assert_equal(
        working_param_dataset["fates_leaf_slatop"].values,
        [4.0, 5.0, 6.0],
    )
...
def test_default_set_value_array_fixed_positions_held(
    ...
    working_param_dataset["fates_leaf_slatop"].values = np.array([1.0, 2.0, 3.0])
    param.set_value(
        working_param_dataset, param_dataset, np.array([7.0, 8.0, 9.0]), {"fates_pft": [0]}
    )
    np.testing.assert_equal(
        working_param_dataset["fates_leaf_slatop"].values,
        [1.0, 8.0, 9.0],
...
def test_default_set_value_all_positions_fixed_returns_original(
    ...
    working_param_dataset["fates_leaf_slatop"].values = np.array([1.0, 2.0, 3.0])
    param.set_value(
        working_param_dataset, param_dataset, np.array([7.0, 8.0, 9.0]), {"fates_pft": [0, 1, 2]}
    )
    np.testing.assert_equal(
        working_param_dataset["fates_leaf_slatop"].values,
...
def test_default_set_value_shape_mismatch_raises(default_row, param_dataset, working_param_dataset):
...
def test_default_set_value_0d_array_input(scalar_row, param_dataset, working_param_dataset):
    ...
    assert float(working_param_dataset["fates_canopy_closure_thresh"].values) == pytest.approx(9.0)


def test_default_set_value_float_input(default_param, param_dataset, working_param_dataset):
...
def test_default_set_value_0d_input(default_param, param_dataset, working_param_dataset):
...
def test_default_set_value_multi_element_array_raises(
    ...
    default_param.active_index = DimIndex("fates_pft", 1)
    with pytest.raises(ValueError, match="scalar"):
        default_param.set_value(working_param_dataset, param_dataset, np.array([1.0, 2.0]))
